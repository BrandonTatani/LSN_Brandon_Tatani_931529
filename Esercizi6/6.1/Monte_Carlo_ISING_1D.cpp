...
{
  int o;
  double p, energy_old, energy_new, sm, A, delta_E;
...
//		delta_E = 2*J*s[o]*(s[Pbc(o-1)] + s[Pbc(o+1)]);
		A = min <double> (1, exp(-beta*(energy_new-energy_old)));
		attempted++;
    ...
    {
		p = 1./(1+exp(-2*beta*J*(s[Pbc(o-1)]+s[Pbc(o+1)])));
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=1;
    ...
    Ene << iblk <<  " " << stima_u << " " << glob_av[iu]/(double)iblk << " " << err_u << " " << glob_av2[iu]/(double)iblk<< endl;
    Ene.close();
...
// INCLUDE YOUR CODE HERE
	...
	stima_c = beta*beta*(blk_av[ic]/blk_norm/(double)nspin - pow(blk_av[iu]/blk_norm, 2)/(double)nspin);
	glob_av[ic] += stima_c;
