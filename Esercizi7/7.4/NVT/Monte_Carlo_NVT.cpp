...
/*	for(int i= 0; i<nbins; i++){
		if(dr <= (box/2.0)){
			if(dr>i*bin_size && dr<(i+1)*bin_size){
				//walker[igofr+i] += 6./(rho*npart*4.*pi*(pow((i+1)*bin_size, 3)-pow(i*bin_size, 3)));
				walker[igofr+i] += 2;
				break;
			}
		}
...
void PrintV(void){
	ofstream pot;
	pot.open("insta_epot.dat", ios::app);
	
	pot << tmp++ << walker[iv] << endl;
	return;
}
...
//g(r)
	...
	for(int k=igofr; k<igofr+nbins; k++){
		r = (double)(k-igofr)*box/((double)nbins*2);
		dr = box/(nbins*2);
		dV = 4.0*M_PI/3.*(pow(r+dr,3)-pow(r,3));
		gdir = blk_av[k]/(double)blk_norm/(rho*npart*dV);
		glob_av[k] += gdir;
		glob_av2[k] += pow(gdir, 2);
		err_gdir=Error(glob_av[k],glob_av2[k],iblk);
		Gofr << bin_size/2.+ (k-igofr)*bin_size << " " << blk_av[k]/blk_norm << " " << endl;
		if(iblk == nblk)
			Gave << bin_size/2.+ (k-igofr)*bin_size <<  " " << glob_av[k]/(double)iblk << " " << err_gdir << endl;
	}
