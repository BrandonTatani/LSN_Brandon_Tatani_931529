...
// -> Class Cromosoma <-
	...
	void Cromosoma :: Set_Gene(int i, int a){
		crom[i] = a;
	}
	
	int* Cromosoma :: Get_Address(int i){
		return &crom[i];
	}
...
//		cout << "Parametri dei cromosomi" << endl;
	...
	double Genetic :: GetL(int i){
		return membro[i].GetL();
	}
	
	
	int Genetic :: Get_Gene(int i, int k){
		return membro[i].Get_Gene(k);
	}
	
	void Genetic :: Set_Gene(int i, int k, int a){
		membro[i].Set_Gene(k, a);
	}
	
	int* Genetic :: Get_Address(int i, int k){
		return membro[i].Get_Address(k);
	}
	void Genetic :: Copy(int* x, int n){
		for(int i=0; i<m_citta->GetN(); i++)
			membro[n].Set_Gene(i, x[i]);
